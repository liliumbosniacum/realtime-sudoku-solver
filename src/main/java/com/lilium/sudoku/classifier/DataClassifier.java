package com.lilium.sudoku.classifier;
...
public class DataClassifier {
    private static final String RESOURCES_FOLDER_PATH = "D:\\Development\\IntelliJ\\realtime-sudoku-solver\\src\\main\\resources\\";
    private static final int HEIGHT = 60;
    private static final int WIDTH = 60;
    private static final int N_SAMPLES_TRAINING = 181; // Adjust according to the number of images in training set
    private static final int N_SAMPLES_TESTING = 9; // Adjust according to the number of images in testing set
    private static final int N_OUTCOMES = 10;

    public static void main(final String args[]) throws IOException {
        BasicConfigurator.configure();

        DataSetIterator dataSetIterator = getDataSetIterator(RESOURCES_FOLDER_PATH + "data\\training", N_SAMPLES_TRAINING);
    ...
    private static void buildModel(DataSetIterator dsi) throws IOException {
        ...
        //Evaluation
        DataSetIterator testDsi = getDataSetIterator(RESOURCES_FOLDER_PATH + "data\\testing", N_SAMPLES_TESTING);
        Evaluation eval = model.evaluate(testDsi);
        System.out.print(eval.stats());

        ModelSerializer.writeModel(model, RESOURCES_FOLDER_PATH + "\\models\\trained.tar", true);
    }
}
